...
// Handles WebSocket communication with local server and CDP-based input simulation
  ...
  try {
    ...
    switch (cmd) {
      ...
      case "extractMap":
        ...
        break;
      case "screenshot":
        result = await cmdScreenshot(msg);
        break;
      case "wait":
      ...
      case "focus":
        ...
        break;
      case "selectOption":
        result = await cmdSelectOption(msg);
        break;
      case "injectCSS":
      ...
      case "injectJS":
        ...
        break;
      case "fill":
        result = await cmdFill(msg);
        break;
      case "scrollIntoView":
        result = await cmdScrollIntoView(msg);
        break;
      default:
...
async function cmdSelectOption(msg) {
  if (!connectedTabId) throw new Error("No browser connected");

  const { selector, ref, value, label } = msg;
  if (!selector && ref === undefined) throw new Error("Either selector or ref must be provided");
  if (value === undefined && label === undefined) throw new Error("Either value or label must be provided");

  const result = await sendToContentScript("selectOption", { selector, ref, value, label });
  return result;
}
...
async function cmdFill(msg) {
  ...
  // Use content script fill which uses native setter + React events
  const result = await sendToContentScript("fill", { selector, value });
  return result;
}

async function cmdScrollIntoView(msg) {
  if (!connectedTabId) throw new Error("No browser connected");

  const { selector, block } = msg;
  if (!selector) throw new Error("Selector is required");

  const result = await sendToContentScript("scrollIntoView", { selector, block });
  return result;
}
...
async function cmdInjectJS(msg) {
  ...
  await attachDebugger(connectedTabId);
  ...
  if (result.exceptionDetails) {
    ...
    const text = err.exception?.description || err.text || "Unknown error";
    return { executed: true, error: text };
  }
...
async function cmdClick(msg) {
  ...
  // If element is obscured (behind sticky header/overlay), use DOM click fallback
  if (coords.fallbackClick) {
    const result = await sendToContentScript("domClick", { selector });
    return { clicked: selector, x: coords.x, y: coords.y, fallback: true, ...result };
  }
...
async function cmdScroll(msg) {
  ...
  if (selector) {
    ...
    // Fallback to hardcoded viewport center (avoids Page.getLayoutMetrics timeout)
    x = 400;
    y = 300;
  }
  ...
  // Wait for browser to process the scroll
...
async function cmdScreenshot(msg = {}) {
  const maxWidth = msg.maxWidth || 1600;
  const maxHeight = msg.maxHeight || 1200;
  const padding = 20;

  // Resolve clip region from ref, selector, or explicit region
  let clip = null;
  if (msg.ref !== undefined) {
    const bounds = await sendToContentScript("getBoundsByRef", { ref: parseInt(msg.ref, 10) });
    if (!bounds.found) {
      throw new Error(bounds.error || `Element not found for ref=${msg.ref}`);
    }
    clip = applyPadding(bounds, padding);
  } else if (msg.selector) {
    const bounds = await sendToContentScript("getBoundsBySelector", { selector: msg.selector });
    if (!bounds.found) {
      throw new Error(bounds.error || `Element not found for selector: ${msg.selector}`);
    }
    clip = applyPadding(bounds, padding);
  } else if (msg.region) {
    clip = { x: msg.region.x, y: msg.region.y, width: msg.region.width, height: msg.region.height, scale: 1 };
  }

  const captureOpts = { format: "png", quality: 80 };
  if (clip) {
    // Get device pixel ratio for correct scaling
    const dpr = await sendToContentScript("getDevicePixelRatio", {}).catch(() => ({ dpr: 1 }));
    clip.scale = dpr.dpr || 1;
    captureOpts.clip = clip;
  }

  const result = await cdp("Page.captureScreenshot", captureOpts);
  ...
  const resizedDataUrl = await blobToBase64(resizedBlob);

  return { image: resizedDataUrl };
}
...
chrome.runtime.onMessage.addListener((msg, sender, sendResponse) => {
  ...
  if (msg.action === "getWebStatus") {
    sendResponse({
      embedEnabled: embedButtonsEnabled,
      connected: !!connectedTabId,
      wsConnected: ws && ws.readyState === WebSocket.OPEN,
    });
    return false;
...
chrome.storage.local.get(["embedButtonsEnabled"], (data) => {
