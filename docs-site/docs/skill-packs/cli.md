# Skill Packs CLI Reference

All commands for installing, managing, and searching skill packs.

## Install Commands

### sidebutton install

Install a skill pack from a local path, git URL, or registry.
...
List all currently installed skill packs.
...
Remove an installed skill pack by domain.
...
Add a skill pack registry and install all its packs.
...
Commands for scaffolding, validating, and publishing skill packs.

### sidebutton init

Scaffold a new skill pack directory with templates.
...
```
  ...
  ✓ Skill pack scaffolded: my-app.example.com
...
Lint a skill pack's structure. Reports errors and warnings.
...
```
  ...
  ✗ Missing _skill.md — every skill pack needs a root skill file.
...
Publish a skill pack to a registry directory. Operates in two modes.
...
| Command | Description |
|---------|-------------|
| `sidebutton install <source>` | Install a skill pack |
| `sidebutton install --list` | List installed packs |
| `sidebutton uninstall <domain>` | Remove a skill pack |
...
| `sidebutton init <domain>` | Scaffold a new skill pack |
